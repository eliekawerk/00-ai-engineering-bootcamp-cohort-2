...
def warehouse_manager_agent_tool_router(state: "State") -> str:
    ...
    if state.warehouse_manager_agent.final_answer:
        return "end"
    elif state.warehouse_manager_agent.iteration > 10:
        return "end"
...
def coordinator_router(state):
    """Decide whether to continue or to end"""
    if state.coordinator_agent.iteration > 10:
        return "end"
    elif (
...
product_qa_agent_tools = [get_formatted_context, get_formatted_reviews_context]
...
warehouse_manager_agent_tool_descriptions = get_tool_descriptions(
    warehouse_manager_agent_tools
)
